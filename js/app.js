...
const QUOTE_RENDERERS = {
  short: { render: renderShortQuote, label: 'quote-short' },
  long: { render: renderPreviewQuote, label: 'quote-preview' },
  deep: { render: renderPreviewQuote, label: 'quote-preview' },
  interpret: { render: renderInterpretQuote, label: 'quote-interpret' },
  debate: { render: renderPreviewQuote, label: 'quote-debate' },
  business: { render: renderBusinessQuote, label: 'quote-business' },
  framework: { render: renderPreviewQuote, label: 'quote-framework' },
  guide: { render: renderPreviewQuote, label: 'quote-guide' },
  negative: { render: renderNegativeQuote, label: 'quote-negative' },
  caution: { render: renderCautionQuote, label: 'quote-caution' }
};
...
function openEditModal(quote) {
  ...
  document.getElementById('editType').value = quote.type || 'short';
...
async function saveEdit() {
  ...
  if (quoteIndex !== -1) {
    ...
    AppState.allQuotes[quoteIndex].type = newType;
    ...
    if (editingQuoteId >= CONFIG.USER_QUOTE_START_ID) {
      ...
      if (userIndex !== -1) {
        ...
        AppState.userQuotes[userIndex].type = newType;
      ...
      if (existingOverride !== -1) {
        ...
        AppState.userQuotes[existingOverride].type = newType;
